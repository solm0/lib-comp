...
export default function ParamKwButton({
  ...
  return (
    keywords && keywords.map((kw, idx) => (
      <button
      key={idx}
      className={`w-auto h-8 text-text-900 font-medium px-3 flex items-center rounded-sm transition-colors duration-300 pointer-events-auto ${currentKeywords.includes(kw) ? "bg-green-500 hover:bg-green-600" : "bg-button-100 hover:bg-button-200"}`}
